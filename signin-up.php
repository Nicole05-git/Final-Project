<?php
   ...
   session_start();
    ...
    if(isset($_POST['submit'])){
        ...
        $sqlSELECT = "SELECT  `email`, `passwordd`,  FROM `login` WHERE email = '$email'";
        ...
        if($result_for_select->num_rows > 0){
            ...
            $check_for_user_acc = "SELECT `potential_userid`, `fname`, `lname`, `age`, `nationalid`, `gender`, `dob`, `nationality`, `tel`, `personal_email` FROM `potential_user_credentialsperson` WHERE personal_email = '$email'";
            $result = $conn->query($check_for_user_acc);

            if($result->num_rows > 0){
                $_SESSION['has_bank_acc'] = TRUE;
            ...
            if($_SESSION['has_bank_acc']){
                $sql_get_bnk_acc = "SELECT `accountno`, `potential_userid`, `account_type`, `balance` FROM `account` WHERE potential_userid = '$userid'";
        ...
        header('Location:\Bankingsys\dashboard\index.html');
        }
        else{
            echo "
            <script>
                                    ...
                                    <div class="login-snip"> <input id="tab-1" type="radio" name="tab" class="sign-in" checked><label for="tab-1" class="tab">Login</label> <input id="tab-2" type="radio" name="tab" class="sign-up"><label for="tab-2" class="tab">SIGN UP</label>
                                        <div class="login-space">
                                            ...
                                            <div class="sign-up-form">
                                                <!--name-->
                                                <div class="group"> <label for="user" class="label">Full name</label> 
                                                    <input name = "full_name" id="user" type="text" class="input" required placeholder="Enter full name"> </div>
                                                <!--email-->
                                                <div class="group"> <label for="pass" class="label">Email Address</label> 
                                                <input name="email" id="pass" type="text" class="input" required placeholder="Enter your email address"> </div>
                                                <!--phone number-->
...
<?php
    ...
    if(isset($_GET['sign_up'])){
        ...
        $sqlINSERT = "INSERT INTO `signup`(`s_email`, `fullname`, `s_passwordd`, `contactt`,) VALUES ('$email', '$user_name','$password', '$phone')";
